...
### Classe Group:
La classe Group rappresenta un gruppo di studio. Ogni gruppo ha un nome (name), un limit di studenti (limit), una lista di studenti (students) e una lista di docenti (lecturers).
- Metodi:
...
class Group:
    ...
    def limit_lecturer(self):
        numero_stud=len(self.student)
        max_prof=numero_stud//10
        prof=round(max_prof)
        if numero_stud<=10:
            prof=1
        return prof
